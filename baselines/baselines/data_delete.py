...
def load_data(name, limit=None, padding=False):
    ...
    if name in ['fb', 'wn']:
        ...
        # Pad subgraphs with [-1,-1,-1]
        if padding:
            # Add a warning here
            import warnings
            warnings.warn('Padding subgraphs with empty triples')

            min_edges = min([len(x) for x in traini + vali + testi])
            max_edges = max([len(x) for x in traini + vali + testi])
        ...
        train, val, test = torch.tensor(traini), torch.tensor(vali), torch.tensor(testi)
        return train, val, test, (n2i, i2n), (r2i, i2r), (min_edges, max_edges)
